...
type DerivedPrivateKeys = { [KeyId in DerivedKeyId]: Tagged<Uint8Array, `${KeyId}-private`> };
type DerivedPublicKeys = { [KeyId in DerivedKeyId]: Tagged<Uint8Array, `${KeyId}-public`> };

export type DerivedPrivateKey<KeyId extends DerivedKeyId> = DerivedPrivateKeys[KeyId];
export type DerivedPublicKey<KeyId extends DerivedKeyId> = DerivedPrivateKeys[KeyId];
...
export class DerivedKeyGenerationService {
  ...
  getKeyPair<KeyId extends DerivedKeyId>(userId: string, keyId: KeyId): DerivedKeyPair<KeyId> {
    ...
    return {
      publicKey: derivedKey.publicKey as DerivedPublicKey<KeyId>,
